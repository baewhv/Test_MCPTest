...
- 모든 버전 관리 작업은 **`.agents/rules/git_rule.md`** 규칙을 100% 준수하여 수행합니다:
  ...
  - **Git Worktree 격리**: `git worktree add ../TestMCP_worktrees/[작업브랜치명] -b [작업브랜치명] develop`
...
1. **신규 작업 요청 수신 시 (Branch / Worktree 준비)**:
   - Developer 또는 타 에이전트로부터 신규 기능 개발 시작 요청을 받으면, 메인 저장소에서 `git worktree add ../TestMCP_worktrees/[작업타입]_[작업명] -b [작업타입]_[작업명] develop` 명령어로 격리된 작업 공간을 생성하고 작업 경로를 안내합니다.
...
3. **Pull Request(PR) 생성 및 상태 갱신**:
   - `develop` 브랜치를 대상으로 GitHub MCP `create_pull_request` 도구를 호출하여 PR을 생성합니다.
   - PR 생성 완료 후 `docs/work/status.md`의 `[현재 상태]`를 갱신합니다:
     - `[현재 상태] [GitManager] [기능명] PR 생성 완료 (PR #nn) ➔ qa에게 검수 인계`
   - `qa` 에이전트에게 검수를 요청합니다.
4. **수정 피드백 수신 시 (PR 갱신)**:
   - 피드백 수정 사항을 워크트리에서 `[fix] : ...` 또는 `[refactor] : ...`로 추가 커밋 및 푸시하여 열려 있는 기존 PR을 자동 갱신합니다.
5. **PR 머지 완료 후 정리**:
   - 사용자가 GitHub에서 PR을 머지하면, `git worktree remove ../TestMCP_worktrees/...` 및 `git branch -d` 명령어로 사용 완료된 워크트리와 브랜치를 깔끔하게 삭제 정리합니다.
   - 정리 완료 후 `docs/work/status.md`의 `[현재 상태]`를 갱신합니다:
     - `[현재 상태] [GitManager] PR 머지 확인 및 Worktree 정리 완료 ➔ 다음 작업 대기`
