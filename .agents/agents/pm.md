...
- **3단계 통제 절차 (Human-in-the-Loop & Worklist Routing)**:
  ...
  2. **2단계 (사용자 검토 및 worklist 최우선 지시 등록 분기)**:
     ...
     - 사용자에게 **"이 이슈를 제안 방향으로 확정하여 `worklist.md`의 최우선 지시 사항에 등록하고 바로 진행할까요? 아니면 백로그에 보류할까요?"** 확인 질문을 던집니다.
  3. **3단계 (사용자 분기 결정에 따른 실행)**:
     - **분기 A (최우선 등록 승인)**: 사용자가 승인("최우선 지시로 등록해서 진행해줘", "방안 A로 지금 바로 고쳐줘")하면, `docs/work/worklist.md` 최상단에 `## 사용자 최우선 지시 사항` 섹션을 신설/등록(`- [ ] [이슈명 수정]`)하고, 비로소 격리 브랜치 생성 및 `Developer ➔ GitManager ➔ QA` 검증 루프를 착수하여 완결 후 `[x]` 체크 처리합니다.
     - **분기 B (보류/차기 반영)**: 사용자가 보류를 선택하면, `docs/work/status.md` 백로그/제안항목에만 기록하고 기존 정규 태스크를 유지합니다.
