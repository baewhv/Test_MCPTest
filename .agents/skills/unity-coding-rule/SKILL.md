...
## 7. 데이터 분리 및 아키텍처 원칙
1. **데이터 분리 (ScriptableObject)**:
   - 정적 기획 수치 및 테이블 데이터는 `[CreateAssetMenu]`를 적용한 `ScriptableObject`(`SO_*`)로 분리하여 관리합니다.
2. **입력 시스템**:
   - New Input System (`UnityEngine.InputSystem`)을 기본으로 사용합니다.
