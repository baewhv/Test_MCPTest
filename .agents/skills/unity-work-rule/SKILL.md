...
description: 유니티 씬 충돌 방지, 독립 완제품 프리팹(PF_*) 우선 조립, 인스펙터 직렬화 바인딩, Missing Reference 방지 및 에디터 스크립팅 제한을 준수하는 유니티 엔진 작업 스킬
---

# 유니티 작업 및 씬/프리팹 조립 표준 스킬 (Unity Work Skill)

이 스킬은 프로젝트의 유니티 에디터 조작, 씬 충돌 방지, 프리팹 조립, 직렬화 바인딩 및 에디터 스크립팅 제한을 규정하는 작업 표준 지침입니다.

---

## 1. 씬 충돌 방지 및 프리팹 우선 조립 원칙 (Scene Conflict Prevention & Prefab-First)
1. **공용 씬 직접 수정 지양**:
   - 개별 기능 개발 브랜치에서는 공용 씬(`*.unity`)을 직접 수정하지 않으며, YAML 머지 충돌을 방지합니다.
2. **독립 완제품 프리팹 우선 정책 (Prefab-First)**:
   - 씬에 배치할 모든 캐릭터, 오브젝트, UI, 스포너는 반드시 **독립 완제품 프리팹(`Assets/Prefabs/PF_[이름].prefab`)** 형태로 조립합니다.
...
3. **표준 해결 원칙**:
   - 모든 기능 구현은 에디터 툴 코드가 아닌 **순수 런타임 표준 컴포넌트(`MonoBehaviour`), 프리팹 우선 조립 및 인스펙터 직렬화 바인딩**으로 완결합니다.
