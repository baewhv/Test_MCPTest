...
## 0. 프로젝트 진입 및 상태 진단 공식 트리거 (Entry & Status Trigger)
- **공식 트리거 키워드**: `"현재 상태는?"` (동의어: `"상태 확인"`, `"status"`)
- 사용자가 트리거를 입력하면 에이전트는 즉시 1단계 레포지토리 명칭 식별을 수행하고 판정 결과에 따라 응답합니다:
  - **마스터 저장소(`Test_MCPTest` 또는 `TestMCP`)**:
    - 마스터 템플릿 모드 상태 보고 (현재 브랜치, operating_pack 보존 상태, 에이전트 거버넌스 보수 대기 안내)
  - **신규 게임 개발 프로젝트**:
    - 즉시 아래 [조건 2]를 자동 실행하여 `GEMINI_SETUP.md`로 헌장을 교체하고, `docs/PROJECT_SPEC.md` 설정 개시를 보고
