...
- **행동 수칙**:
  ...
  3. 모든 에이전트 및 스킬의 수정, 추가, 최적화는 `.agents/templates/operating_pack/` 하위에서 직접 수행하며, 상위 `.agents/agents/` 및 `.agents/skills/`는 토큰 격리 및 클린 배포 상태 유지를 위해 비워둡니다.
...
- **행동 수칙 (즉시 GEMINI_SETUP 전환 및 운영 에셋 격리)**:
  ...
  2. **운영 에셋 격리(Clean Isolation)**:
     ...
     - 셋업 단계에서 불필요한 스킬/에이전트의 자동 로드(Auto-Discovery) 및 토큰 소모를 방지하기 위해 상위 폴더가 비어 있는 상태를 유지합니다.
