...
- **행동 수칙**:
  ...
  3. 다른 헌장으로 임의 전환하지 않고, 본 헌장 및 `.agents/templates/` 하위 템플릿의 정밀도를 유지합니다.

---

### [조건 2: 레포지토리 명칭이 다른 명칭인 경우 (신규 프로젝트로 복제된 경우)]
- **판정**: **신규 게임 개발 프로젝트 (New Game Development Project)**
- **행동 수칙 (즉시 GEMINI_SETUP 자동 전환)**:
  1. 에이전트는 즉시 프로젝트 루트의 `GEMINI.md`를 아래 파일의 내용으로 완전히 덮어씁니다:
     - **소스 파일**: `.agents/templates/GEMINI_SETUP.md`
     - **대상 파일**: 프로젝트 루트의 `GEMINI.md`
  2. 덮어쓰기가 완료되면 시스템은 **신규 프로젝트 초기 환경 설정 모드(Setup Mode)**로 진입합니다.
  3. 에이전트는 사용자에게 다음과 같이 보고하고 초기 셋업을 시작합니다:
