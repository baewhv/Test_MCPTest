...
### [2단계: 4단계 아키텍처 우선(Architecture-First) 구현 & 1태스크 엄수 원칙]
1. **철저한 1태스크 범위 엄수 (Strict Single-Task Scope & Anti-Scope-Creep)**:
   - Developer는 PM으로부터 지시받은 **오직 1개의 태스크(`docs/work/worklist.md`의 지정 항목) 범위에 해당하는 코드와 프리팹만 정확히 구현**합니다.
   - 후속 태스크(Task N+1, N+2 등)에 포함된 기능이나 미할당 로직을 한 번에 미리 구현(Scope Creep)하는 행위를 엄격히 금지합니다.
   - 단위 구현 범위를 최소화해야만 PR 타임라인 댓글과 QA NUnit 단위 테스트의 신뢰성이 100% 보장됩니다.
2. **토큰 절약형 더미 리소스 우선 원칙 (Primitive First)**:
...
3. 아래 의존성 순서에 따라 C# 스크립트와 프리팹을 조립합니다:
